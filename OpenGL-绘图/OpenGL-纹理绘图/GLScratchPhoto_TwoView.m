...
@interface GLScratchPhoto_TwoView ()
{
...
@implementation GLScratchPhoto_TwoView
...
- (void)drawingOpenGL:(UIImage *)image {
    // 将image绑定到GL_TEXTURE_2D上，即传递到GPU中
    _BGImagetexture = [self setupTexture:image];
...
- (void)renderUsingIndexVBO:(CGPoint)point {
    ...
    UIImage *image1 = [UIImage imageNamed:@"magic_radial"];
    [self didDrawImageViaOpenGLES:image1];
...
#warning 可以删除纹理，可是达不到目的效果
//    glScissor(current.x,current.y,50,50);
