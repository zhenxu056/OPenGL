...
@interface MTGLDrawView () {
    ...
    GLuint _drawPositionSlot;
    GLuint _drawTexture;
    GLuint _drawTextureCoordsSlot;
    GLuint _drawTextureUniform;
    
    //用于涂抹的图片
    GLuint _PaintImagetexture;
    GLuint _PaintImagetextureCoordsSlot;
    GLuint _PaintImagepositionSlot;
    ...
    GLuint _bgTextureUniform;

    
    GLuint _BGProgramHandle;
...
- (void)setBackGroundImage {
    ...
    _BGImagetexture = [MTOpenGLManner setupTexture:self.bgImage
                                isDeletBindTexture:NO];

    //绑定纹理操作
    [MTOpenGLManner texture:_BGImagetexture
              activeTexture:GL_TEXTURE0
                  TextureID:0
                    uniform:_bgTextureUniform];
    ...
    glVertexAttribPointer(_BGImagetextureCoordsSlot, 2, GL_FLOAT, GL_FALSE, 0, texCoords1);
    ...
    glEnableVertexAttribArray(_BGImagepositionSlot);
    glVertexAttribPointer(_BGImagepositionSlot, 2, GL_FLOAT, GL_FALSE, 0, vertices3);
    ...
    _drawTexture = [MTOpenGLManner setupTexture:self.drawImage
                             isDeletBindTexture:NO];
    
    _PaintImagetexture = [MTOpenGLManner setupTexture:self.paintImage
                                   isDeletBindTexture:NO];
}
...
- (void)renderUsingIndexVBO:(CGPoint)point {
    //绑定纹理操作
    [MTOpenGLManner texture:_PaintImagetexture activeTexture:GL_TEXTURE1 TextureID:1 uniform:_paintTextureUniform];
    
    //绑定纹理操作
    [MTOpenGLManner texture:_drawTexture activeTexture:GL_TEXTURE2 TextureID:2 uniform:_drawTextureUniform];
    ...
    glVertexAttribPointer(_drawTextureCoordsSlot, 2, GL_FLOAT, GL_FALSE, 0, texCoords);
    glEnableVertexAttribArray(_drawTextureCoordsSlot);
    ...
    glVertexAttribPointer(_PaintImagetextureCoordsSlot, 2, GL_FLOAT, GL_FALSE, 0, texCoords1);
    glEnableVertexAttribArray(_PaintImagetextureCoordsSlot);
    ...
    glEnableVertexAttribArray(_PaintImagepositionSlot);
    glVertexAttribPointer(_PaintImagepositionSlot, 2, GL_FLOAT, GL_FALSE, 0, vertices3);
    glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);
    
    
//    glBindTexture(GL_TEXTURE_2D, 0);
//    glDeleteTextures(1, &_PaintImagetexture);
...
- (void)afferentTextureData {  //传入绘图纹理图片的坐标
    _paintTextureUniform = glGetUniformLocation(_programHandle, "Texture");
    _PaintImagepositionSlot = glGetAttribLocation(_programHandle, "Position");
    _PaintImagetextureCoordsSlot = glGetAttribLocation(_programHandle, "TextureCoords");
    
    _drawTextureUniform = glGetUniformLocation(_programHandle, "BGImageTexture");
    _drawPositionSlot = glGetAttribLocation(_programHandle, "BGImagePosition");
    _drawTextureCoordsSlot = glGetAttribLocation(_programHandle, "BGImageTextureCoords");
}

- (void)afferentBGTextureData {
    _bgTextureUniform = glGetUniformLocation(_BGProgramHandle, "BGTexture");
    _BGImagepositionSlot = glGetAttribLocation(_BGProgramHandle, "BGPosition");
    _BGImagetextureCoordsSlot = glGetAttribLocation(_BGProgramHandle, "BGTextureCoords");
}
...
- (void)compilingBGShader {
    ...
    _BGProgramHandle = [MTOpenGLManner compileShaders:shaderVertex1 shaderFragment:shaderFragment1];
    //设置纹理的一些数据
...
- (void)compilingDrawShader {
    ...
    _programHandle = [MTOpenGLManner compileShaders:shaderVertex1 shaderFragment:shaderFragment1];
    //设置纹理的一些数据
